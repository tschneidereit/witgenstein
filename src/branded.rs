...
//! from the `ts-auto-wit` package to build a map of type name → WIT type.
...
/// Scan a program's AST for branded type definitions and ts-auto-wit imports.
...
impl<'a> Visit<'a> for BrandedTypeCollector {
    fn visit_import_declaration(&mut self, decl: &oxc_ast::ast::ImportDeclaration<'a>) {
        // Detect `import { u32, s64, ... } from 'ts-auto-wit'`
        if decl.source.value.as_str() != "ts-auto-wit" {
            return;
        }
